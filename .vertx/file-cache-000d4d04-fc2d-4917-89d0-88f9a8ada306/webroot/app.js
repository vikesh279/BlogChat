...
(function($) {
	...
	app.controller('AppHomeController', function($http, $log, $scope,
		...
		$http.get('http://localhost:9090/Services/rest/blogs').success(
		...
		$http.get('/Services/rest/user?signedIn=true').success(
			...
			$scope.tagSearch = function(){
				$http.get('http://localhost:9090/Services/rest/blogs?tag='+$scope.searchTag).success(
					function(data, status, headers, config) {
						$scope.blogs = data;
						$scope.loading = false;
					}).error(function(data, status, headers, config) {
			...
			$scope.submitComment = function(comment, blogId){
				...
				$http.post('http://localhost:9090/Services/rest/blogs/'+blogId+'/comments',comment).success(
					function(data, status, headers, config) {
						$scope.loading = false;
	...
	app.controller('LoginController', function($http, $log, $scope, $location,
		...
		$http.get('http://localhost:9090/Services/rest/company').success(
				function(data, status, headers, config) {
		...
		$scope.companyChange = function(companyId) {
			...
			// Load sites
			$http.get('http://localhost:9090/Services/rest/company/'+companyId+'/sites').success(
					function(data, status, headers, config) {
						$scope.sites = data;
		...
		$scope.siteChange = function(companyId, siteId) {
			...
			// Load sites
			$http.get('http://localhost:9090/Services/rest/company/'+companyId+'/sites/'+siteId+'/departments').success(
					function(data, status, headers, config) {
